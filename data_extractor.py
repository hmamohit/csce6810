...
def balance_loaded_data(
    ...
    return weights[kept_idx], coords[kept_idx], targets[kept_idx], labels[kept_idx], original_indices[kept_idx]
...
def main():
    ...
    labels = make_labels(targets, mode="fixed_thresholds",
                         low_max=0.01, med_max=1.0)
    original_indices = np.arange(len(targets), dtype=np.int64)

    weights, coords, targets, labels, original_indices = balance_loaded_data(
        weights,
        coords,
        targets,
        labels,
    ...
    np.savetxt(processed_dir / "balanced_indices.txt",
               original_indices, fmt="%d")
    np.savetxt(processed_dir / "balanced_weights.txt", weights, fmt="%.6f")
    np.savetxt(processed_dir / "balanced_coords.txt", coords, fmt="%.6f")
    np.savetxt(processed_dir / "balanced_targets.txt", targets, fmt="%.6f")
    np.savetxt(processed_dir / "balanced_labels.txt", labels, fmt="%d")
